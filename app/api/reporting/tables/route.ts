...
// Helper to get DB connection from env
function getDbConnection() {
  const url = process.env.DATABASE_URL;
  if (!url) throw new Error('DATABASE_URL not set');
  // Parse the URL to extract connection info
...
export async function POST(req: NextRequest) {
  // Proxy to /api/v1/reports/tables of reporting engine
  const dbConn = getDbConnection();
  const reportingUrl = process.env.REPORTING_ENGINE_URL || 'http://localhost:8080';
  const res = await fetch(`${reportingUrl}/api/v1/reports/tables`, {
    method: 'POST',
