    ...
    return (
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-6">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Project Tasks</h3>
                <button
            ...
            <div className="space-y-4">
                <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-4">Recent Tasks</h4>
                ...
                {project.tasks && project.tasks.length > 0 ? (
                    <div className="space-y-3">
                        {project.tasks
                            ...
                            .map((task) => {
                                ...
                                return (
                                    <div key={task.task_id} className="border border-gray-200 dark:border-slate-700 rounded-lg p-4 hover:shadow-md transition-shadow">
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1">
                                                <div className="flex items-center space-x-3 mb-2">
                                                    <h5 className="font-medium text-gray-900 dark:text-gray-100">{task.name}</h5>
                                                    {task.is_milestone && (
                                                        <span className="px-2 py-1 bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-300 text-xs rounded-full">Milestone</span>
                                                    )}
                                                    <span className={`px-2 py-1 rounded-md text-xs font-medium ${task.priority === "high" ? "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300" : task.priority === "medium" ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300" : "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300"}`}>
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className={`font-medium ${task.status === "completed" ? "text-green-600" : task.status === "in_progress" ? "text-blue-600" : task.status === "on_hold" ? "text-red-600" : "text-gray-600"}`}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center space-x-2 mt-1">
                                                            ...
                                                            <div className="flex-1 bg-gray-200 dark:bg-slate-700 rounded-full h-2 min-w-[40px]">
                                                                <div
                                                                    className={`h-2 rounded-full transition-all duration-300 ${task.status === "completed" ? "bg-green-500" : task.status === "in_progress" ? "bg-blue-500" : isOverdue ? "bg-red-500" : "bg-gray-400"}`}
                    ...
                    <div className="text-center py-8 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-lg">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-4">This project doesn't have any tasks created yet.</p>
                        <button onClick={() => router.push(`/projects/${projectId}/tasks`)} className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors">
                            Create First Task
                        </button>
                    </div>
                )}
                ...
                {project.tasks && project.tasks.length > 5 && (
                    <div className="text-center pt-4 border-t border-gray-200 dark:border-slate-700">
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Showing 5 of {project.tasks.length} tasks</p>
                        <button onClick={() => router.push(`/projects/${projectId}/tasks`)} className="text-orange-600 hover:text-orange-700 font-medium">
