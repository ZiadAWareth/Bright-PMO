...
export function LandingHeroPreview() {
  return (
    <div className="relative">
      ...
      <div
        aria-hidden="true"
        className="absolute -inset-6 -z-10 rounded-[2rem] bg-bright-primary/20 blur-3xl"
      />
      ...
      <div className="rounded-3xl border border-border bg-bg-surface/90 p-5 shadow-2xl backdrop-blur">
        ...
        <div className="grid grid-cols-2 gap-3">
          {KPIS.map(({ label, value, Icon }) => (
            ...
            >
              <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-lg bg-bright-primary/10 text-bright-primary">
        ...
        <div className="mt-3 rounded-2xl border border-border bg-bg-surface-alt/70 p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-1 text-xs font-semibold text-bright-primary">
          ...
          <div className="flex h-24 items-end gap-1.5">
            {BARS.map((height, index) => (
              <div
                key={index}
                className="flex-1 rounded-t-md bg-gradient-to-t from-bright-primary/40 to-bright-primary"
                style={{ height: `${height}%` }}
              />
            ))}
          </div>
        ...
        <div className="absolute -bottom-7 -right-6 flex animate-float-soft items-center gap-2.5 rounded-2xl border border-border bg-bg-surface p-3 shadow-xl">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-bright-success/10 text-bright-success">
