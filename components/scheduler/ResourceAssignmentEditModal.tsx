...
const EditAssignmentModal = ({
    ...
    const handleInputChange = (field: string, value: any) => {
        setFormData((prev) => {
            ...
            // No longer auto-calculate fields based on each other
            // Allocation percentage and planned hours are now independent
    ...
    const validateForm = () => {
        ...
        // Task date boundary validations
        if (formData.start_date && formData.end_date) {
            ...
            if (assignmentStartDate < taskStartDate) {
                newErrors.start_date = `Assignment start date cannot be before task start date (${new Date(
                    task.start_date
                ).toLocaleDateString("en-GB")})`;
            }

            if (assignmentEndDate > taskEndDate) {
                newErrors.end_date = `Assignment end date cannot be after task end date (${new Date(
                    task.end_date
                ).toLocaleDateString("en-GB")})`;
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const assignmentData = {
                ...assignment, // Preserve existing fields
                ...formData,
            };
